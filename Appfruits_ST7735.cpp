 ...
 ****************************************************/
  ...
  /*
 * We made changes and introduced a backbuffer (possible on Teensy as it
 * has more SRAM than typical AVR MCUs) that allows for much faster
 * refresh rates (only pixels that have changed are transferred via SPI)
 * We also added Portrait and Landscape modes.
 *
 * This is a drop-in replacement for the existing Adafruit_ST7735 library.
 * Give it a try if your hardware supports it.
 * 
...
Appfruits_ST7735::Appfruits_ST7735(uint8_t cs, uint8_t rs, uint8_t sid,
...
Appfruits_ST7735::Appfruits_ST7735(uint8_t cs, uint8_t rs, uint8_t rst) :
  ...
  _clipRect = Rect(0,0,ST7735_TFTWIDTH,ST7735_TFTHEIGHT);
}
...
/***************************************************************/
#ifdef __AVR__
inline void Appfruits_ST7735::writebegin()
{
}

inline void Appfruits_ST7735::spiwrite(uint8_t c)
{
...
void Appfruits_ST7735::writecommand(uint8_t c)
{
  *rsport &= ~rspinmask;
  *csport &= ~cspinmask;
  //Serial.print("C ");
...
void Appfruits_ST7735::writedata(uint8_t c)
{
  *rsport |=  rspinmask;
  *csport &= ~cspinmask;
  //Serial.print("D ");
...
void Appfruits_ST7735::writedata16(uint16_t d)
{
  *rsport |=  rspinmask;
  *csport &= ~cspinmask;
  //Serial.print("D ");
...
void Appfruits_ST7735::setBitrate(uint32_t n)
{
  if (n >= 8000000) {
...
void Appfruits_ST7735::commandList(const uint8_t *addr) {
...
void Appfruits_ST7735::commonInit(const uint8_t *cmdList)
{
...
void Appfruits_ST7735::initB(void) {
  commonInit(Bcmd);
}


// Initialization for ST7735R screens (green or red tabs)
void Appfruits_ST7735::initR(uint8_t options) {
  commonInit(Rcmd1);
...
void Appfruits_ST7735::setAddrWindow(uint8_t x0, uint8_t y0, uint8_t x1,
 uint8_t y1) {
...
void Appfruits_ST7735::pushColor(uint16_t color)
{
  writebegin();
  writedata16(color);
}

void Appfruits_ST7735::setPixel(int16_t x, int16_t y, uint16_t color)
{
  ...
  else if (_orientation == PORTRAIT_UPSIDEDOWN)
  {
    x1 = x;
    y1 = _height - y;
  }
  else if (_orientation == LANDSCAPE_LEFT)
  {
    x1 = x * _transform[0][0] - y * _transform[0][1];
    y1 = x * _transform[1][0] + y * _transform[1][1];

    x1 = _width - (_width + x1);
    y1 += _height;
  }
  else if (_orientation == LANDSCAPE_RIGHT)
  {
    x1 = x * _transform[0][0] - y * _transform[0][1];
    y1 = x * _transform[1][0] + y * _transform[1][1];

    x1 += _width;
    y1 = _height-(_height + y1);
  }

  if((x1 < 0) ||(x1 >= _width) || (y1 < 0) || (y1 >= _height)) return;

  //TODO: We must transform cliprect!
  if (x1 < _clipRect.x || x1 > _clipRect.right())
  {
    return;
  }

  if (y1 < _clipRect.y || y1 > _clipRect.bottom()-1)
  {
    return;
  }

  if (this->backbuffer[x1][y1] != color)
  {
    this->backbuffer[x1][y1] = color;
    if (x1 < _dirtyX) _dirtyX = x1;
    if (x1 > _dirtyX1) _dirtyX1 = x1;
    if (y1 < _dirtyY) _dirtyY = y1;
    if (y1 > _dirtyY1) _dirtyY1 = y1;
  }
}

void Appfruits_ST7735::drawPixel(int16_t x, int16_t y, uint16_t color)
{
  setPixel(x,y,color);
}

void Appfruits_ST7735::display()
{
  if (_dirtyX > _dirtyX1)
  {
#if DEBUG_DRAW_CYCLE
...
#if DEBUG_DRAW_CYCLE
  drawRect(_dirtyX+1,_dirtyY+1,_dirtyX1-_dirtyX-1,_dirtyY1-_dirtyY-1,ST7735_RED);
#endif

  setAddrWindow(0,_height-_dirtyY1-1,_width-1,_height-_dirtyY-1);

  for(int y=_height-1-_dirtyY; y>=_height-1-_dirtyY1; y--) {
    for(int x=_width; x>0; x--) {
        writedata16(this->backbuffer[x-1][y-(_height-1-_dirtyY1-_dirtyY)]);
      //writedata16(ST7735_CYAN);
    }
  }
...
void Appfruits_ST7735::clear(uint16_t color)
{
  //Clear bit mask
  _dirtyX = 0;
  _dirtyX1 = _width-1;
  ...
  memset(this->backbuffer,color,_width*_height*sizeof(uint16_t));
}

void Appfruits_ST7735::drawFastVLine(int16_t x, int16_t y, int16_t h,
 uint16_t color) {
...
/*  setAddrWindow(x, y, x, y+h-1);
  ...
  while (h--) {
    setPixel(x,y+h,color);
  }
}


void Appfruits_ST7735::drawFastHLine(int16_t x, int16_t y, int16_t w,
  uint16_t color) {
...
void Appfruits_ST7735::fillScreen(uint16_t color) {
...
// fill a rectangle
void Appfruits_ST7735::fillRect(int16_t x, int16_t y, int16_t w, int16_t h,
  uint16_t color) {
...
void Appfruits_ST7735::fillRect(Rect frame, uint16_t color) {
  fillRect(frame.x,frame.y,frame.width,frame.height,color);
}
...
void Appfruits_ST7735::setRotation(uint8_t m) {
...
void Appfruits_ST7735::invertDisplay(boolean i) {
  writecommand(i ? ST7735_INVON : ST7735_INVOFF);
}


uint16_t Appfruits_ST7735::calcTextWidth(String text, uint8_t font)
{
  uint16_t width = 0;
...
uint8_t Appfruits_ST7735::calcTextHeight(String text, uint8_t font)
{
  return fontDesc['Q'].height;
}

void Appfruits_ST7735::setOrientation(uint8_t orientation)
{
  _orientation = orientation;

  if (orientation == PORTRAIT || orientation == PORTRAIT_UPSIDEDOWN)
  {
    //Identity
    _transform[0][0] = 1;
    _transform[0][1] = 0;
...
void Appfruits_ST7735::drawIcon(Icon *icon, uint8_t x, uint8_t y, uint16_t color)
{
  int h = icon->getHeight();
  int w = icon->getWidth();
  const uint8_t* pixelData = icon->getPixelData();
  for (int y1=0;y1<h;y1++)
  {
    for (int x1=0;x1<w;x1++)
    {
      if (pixelData[y1*w+x1] == 1)
      {
...
void Appfruits_ST7735::setClipRect(Rect rect)
{
  _clipRect = rect;
}

void Appfruits_ST7735::resetClipRect()
{
  _clipRect = Rect(0,0,ST7735_TFTWIDTH,ST7735_TFTHEIGHT);
}
